...
### Модель работает по расписанию

На текущей установке `llama-server` поднят **05:00–23:00 по Душанбе** и погашен
ночью, чтобы разгрузить узел. Вне окна `status()` вернёт `available: false`, а
`complete()` — ошибку `503` с текстом «языковая модель не отвечает». Это штатное
состояние, а не поломка вызывающего.

Задание, поставленное через `submit()` при спящей модели, **не ждёт утра**: оно
завершится со `state: "JOB_STATE_FAILED"`. Повторять его в рабочем окне должен
планировщик на вашей стороне.

```ts
const st = await mg.ai.status();
if (!st.available) {
  // не пытайтесь — поставьте задачу в свою очередь и вернитесь позже
  return;
}
```
