...
- `client.ai` — доступ к языковой модели платформы (`AIResource`).
  - `complete()` — короткий синхронный вызов;
  - `submit()` + `wait()` — длинные задания: модель обслуживает один запрос за
    раз, и синхронный вызов на большом входе упирается в таймаут шлюза;
  - `embed()` — векторы для поиска по смыслу;
  - `status()` — доступность, модель и глубина очереди.
- Типы `AICompleteRequest`, `AICompleteResponse`, `AICompletionJob`,
  `AIEmbedResponse`, `AIJobState`, `AIStatus`, `AISubmitRequest`,
  `AIWaitOptions`.

### Notes

- Модуль `mgAI` должен быть включён тенанту, иначе вызовы дают 403.
- Системную часть промпта держите неизменной между вызовами одного сценария:
  включено переиспользование кеша префилла, и подстановка туда даты или имени
  пользователя тихо ломает кеш.
...
- New optional `sponsor_wallet_id` on the `withdraw` request body. When
  set, the platform sponsor wallet pays the chain fee + ATA rent instead
  of the source.
- 1 new vitest mock-fetch test (12 total walletAdmin).

Server-side: dual-signer Solana tx (sponsor first, source second). Phase 1
only — fee debit from sponsor's off-chain balance deferred to status-sync
worker (Phase 2).
