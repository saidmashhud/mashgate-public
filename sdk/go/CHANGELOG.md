...
- Сгенерированный клиент переехал из `_generated/` в `mashgatev1/`. Каталоги на
  подчёркивание Go пропускает: пакета не видели ни `go mod tidy`, ни
  `go build ./...`, поэтому зависимость `github.com/oapi-codegen/runtime` не
  попадала в `go.mod`. Импорт по явному пути при этом работал, а сборка падала
  на «no required module provides package» — то есть клиент из v1.19.0 был
  собран, опубликован и **непригоден к использованию**.

### Changed

- **Ломающее:** минимальная версия Go поднята с 1.22 до 1.24.0. Планку задаёт
  `oapi-codegen/runtime` v1.7.0, без которого сгенерированный клиент не
  работает; выбора между «старый Go» и «весь контракт» нет.
- Импорт: `github.com/saidmashhud/mashgate-public/sdk/go/mashgatev1`
  вместо `.../_generated`. Обёртки в корне пакета `mashgate` не изменились.
- Примеры переведены на Go 1.24, чтобы репозиторий оставался согласованным.

## [v1.19.0] - 2026-08-30 - `sdk/go/v1.19.0`

### Added

- Клиент на **каждую** операцию контракта (633 операции, 1266 методов), а не
  только на те, что успели обернуть руками: генератор переключён на
  `client: true`. Раньше из 397 операций обёрнуто было 110.

### Note

Версия непригодна к использованию — см. v1.20.0. Тег оставлен, чтобы история
модуля не имела дыр.
